"""
Copy and modified from https://github.com/Skyvern-AI/skyvern. Remove the 
"""
...
class BrowserManager:
    ...
    async def get_or_create_for_task(self, task: Task) -> BrowserState:
        ...
        browser_state = await self._create_browser_state(
            task.proxy_location, task.url, task.task_id
        )
        ...
        if task.workflow_run_id and task.workflow_run_id in self.pages:
            if self.pages[task.workflow_run_id].browser_artifacts.video_artifact_id:
                LOG.warning(
                    ...
                    workflow_run_id=task.workflow_run_id,
                    old_artifact_id=self.pages[
                        task.workflow_run_id
                    ].browser_artifacts.video_artifact_id,
                    new_artifact_id=artifact_id,
                )
            self.pages[task.workflow_run_id].browser_artifacts.video_artifact_id = (
                artifact_id
            )
            return
        if task.task_id in self.pages:
